(function(){
	...
	app.controller('MainController',['$http', '$timeout', function($http, $timeout){
		...
		var inUsername = document.getElementById('inUsername').value;
		var inPassword = document.getElementById('inPassword').value;
		var inUsernameFriend = document.getElementById('inUsernameFriend').value;
		...
		this.init = function() {
			...
			$http.defaults.headers.common['Authorization'] = 'Basic ' + window.btoa(inUsername + ':' + inPassword);
			$http.get('api/user/friend/' + inUsernameFriend).success(function(data) {
				main.friend = data;
			}).error(function(data) {
	            alert('Error!!!');
	        });
			...
			console.log(inUsernameFriend);
			$http.defaults.headers.common['Authorization'] = 'Basic ' + window.btoa(inUsername + ':' + inPassword);
			$http.post('api/chat/username', {Username1: inUsername, Username2: inUsernameFriend}).success(function(data) {
				main.chat = data;
				console.log(data);
			}).error(function(data) {
	            alert('Error!!!');
	        ...
	        var socket = io.connect('http://localhost:3000');
	        setTimeout(function() {
				socket.on(main.user._id + 'message', function(data) {	
					$timeout(function(){
	    ...
	    this.sendMessage = function(){
    		$http.defaults.headers.common['Authorization'] = 'Basic ' + window.btoa(inUsername + ':' + inPassword);
    		$http.put('api/chat/addMessage/' + main.chat._id, {Username: inUsername, messageContent: main.messageContent}).success(function(data) {
    			$http.defaults.headers.common['Authorization'] = 'Basic ' + window.btoa(inUsername + ':' + inPassword);
				$http.post('api/chat/username', {Username1: inUsername, Username2: inUsernameFriend}).success(function(data) {
					main.chat = data;
					var elem = document.getElementById('chat-box');
